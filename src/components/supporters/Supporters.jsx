import React from "react";

const Supporters = () => {
  const item = [
    {
      id: 1,
      img: "https://www.dhanalakshmijewellers.com/wp-content/uploads/2019/12/logo.png",
    },
    {
      id: 2,
      img: "data:image/svg+xml;base64,",
    },
    { id: 3, img: "https://zeevector.com/wp-content/uploads/Caratlane-logo-PNG-Transparent.png" },
    { id: 4, img: "https://www.kalyanjewellers.net/images/kalyan_logo.svg" },
    { id: 5, img: "https://www.giva.co/cdn/shop/files/IMG-1171-min_180x.png?v=1690403034" },
    { id: 6, img: "https://www.shilpalifestyle.com/front_assets/img/logo.png" },
    { id: 7, img: "https://www.voylla.com/cdn/shop/files/Voylla_Logo_480_x_120_012ca862-d77b-423a-a284-574a626a096e.webp?v=1656497857" },
    { id: 8, img: "https://cf-cdn.pcjeweller.com/public/uploads/settings/pcj-logo__592261435__1103462098.png" },
    { id: 9, img: "https://kinclimg5.bluestone.com/f_webp/static/resources/themes/bluestone/images/new/bs-logo-2x.png" },
  ];

  return (
    <>
      <div className="w-100 my-[100px]">
        <div className="text-center">
          <p className="my-5 fs-1 text-center gallient-font fw-bold">
            ~ Our Supporters ~
          </p>
        </div>
        <div className="w-[80%] mx-auto flex flex-wrap gap-3 justify-center">
          {item.map((item) => (
            <div
              key={item}
              className="flex w-[130px] h-[80px] shadow-lg rounded-xl"
            >
              <div className="w-100 h-fit p-4 lg:p-5 align-self-center justify-center align-items-center">
                <img
                  src={item.img}
                  alt=""
                />
              </div>
            </div>
          ))}
        </div>
      </div>
    </>
  );
};

export default Supporters;
