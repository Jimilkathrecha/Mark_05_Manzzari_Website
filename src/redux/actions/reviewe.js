...
export const updateReviewAction = (updateReviewData) => {
    return (dispatch) => {
        return ApiPut(`/api/v1/review/update-review/${updateReviewData._id}`, updateReviewData)
            .then((res) => {
                if (res?.status === 200) {
                    dispatch({
                        type: UPDATE_REVIEW,
...
export const approveReviewAction = (approveReviewData) => {
    return (dispatch) => {
        return ApiPut(`/api/v1/review/approve-review/${approveReviewData._id}`, approveReviewData)
            .then((res) => {
                if (res?.status === 200) {
                    dispatch({
                        type: APPROVE_REVIEW,
